...
def list_experiments(
    project_id: int = Query(..., description="Project owning the experiments"),
    db: Session = Depends(get_db),  # noqa: B008
) -> list[Experiment]:
    """List experiments within a project."""
...
def get_experiment(
    experiment_id: int,
    project_id: int = Query(..., description="Project owning the experiment"),
    db: Session = Depends(get_db),  # noqa: B008
) -> Experiment:
    """Retrieve a single experiment within a project."""
    logger.info(
        "Fetching experiment experiment_id=%d project_id=%d",
        experiment_id,
        project_id,
    )
    return ExperimentService(db).get_experiment(experiment_id, project_id)
...
def update_experiment(
    ...
    """Partially update an experiment within a project."""
    logger.info(
        "Updating experiment experiment_id=%d project_id=%d",
        experiment_id,
        project_id,
    )
    return ExperimentService(db).update_experiment(experiment_id, project_id, payload)
...
def delete_experiment(
    ...
    """Delete an experiment within a project, cascading to its runs."""
    logger.info(
        "Deleting experiment experiment_id=%d project_id=%d",
        experiment_id,
        project_id,
    )
    ExperimentService(db).delete_experiment(experiment_id, project_id)
